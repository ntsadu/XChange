...
export class FetchingService {
  ...
  getStockData(apiFunction: string, apiSymbol: string, apiParser: string[], apiInterval?: string): Stock {
    ...
    if (!apiInterval) {
      ...
      this.http.get(`${this.API_URL}${apiFunction}&symbol=${apiSymbol}&${this.API_KEY}`)
      ...
      let promise = new Promise((resolve, reject) => {
        this.http.get(`${this.API_URL}${apiFunction}&symbol=${apiSymbol}&${apiInterval}&${this.API_KEY}`)
          .toPromise()
          .then(response => {
